...
sources = [mesh_source_sv] #[mesh_source_vc, mesh_source_mi, mesh_source_sj] [mesh_source_sv, mesh_source_vc, mesh_source_mi, mesh_source_sj]
...
def process(slice):
    ...
    for i, ssv_id in enumerate(ssv_ids[slice]):
        ...
        # load each obj_type
        for j, obj_type in enumerate(['sv']): # enumerate(['vc', 'mi','sj']): # add SVs here if desired
            if obj_type == 'vc':
                mesh = ssv.vc_mesh
            elif obj_type == 'mi':
                mesh = ssv.mi_mesh
