...
mesh_meta = PrecomputedMetadata(
    cloudpath, config, cache, volume_info
)
mesh_source_sv = PrecomputedMeshSource(mesh_meta, cache, config)
mesh_source_vc = PrecomputedMeshSource(mesh_meta, cache, config)
mesh_source_mi = PrecomputedMeshSource(mesh_meta, cache, config)
mesh_source_sj = PrecomputedMeshSource(mesh_meta, cache, config)
...
def process(slice):
    ...
    for i, ssv_id in enumerate(ssv_ids[slice]):
        ...
        if i % 10 == 0:
            print(f'Computing... {10*i} of slice')            

        ssv.load_attr_dict()

        # load each obj_type
        for j, obj_type in enumerate(['sv']): #, 'vc', 'mi','sj']): # 'syn_ssv'
