...
cloudpath = 'file:///dev/shm/tmpam/assembled_core/'
global_params.wd = '/wholebrain/songbird/j0126/ssdscratch_wds/assembled_core_relabeled_base_merges_relabeled_to_v4b_base_20180214_full_agglo_cbsplit_with_reconnects_no_soma_merger_manual_edges_removed'
...
def process(slice):
    ...
    for i, ssv_id in enumerate(ssv_ids[slice]):
        ...
        # load each obj_type
        for j, obj_type in enumerate(['sv','vc','mi','sj']): # enumerate(['vc', 'mi','sj']): # add SVs here if desired
            if obj_type == 'vc':
                mesh = ssv.vc_mesh
            elif obj_type == 'mi':
                mesh = ssv.mi_mesh
